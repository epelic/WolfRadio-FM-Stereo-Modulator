...
public partial class MainWindow : Window
{
    ...
    async void Start_Click(object sender, RoutedEventArgs e)
    {
        ...
        try {
            ...
            IAudioSource source = selected.Id switch { TestToneId=>new TestToneSource(),FileAudioId when audioFile!=null=>new FileAudioSource(audioFile),FileAudioId=>throw new InvalidOperationException("Select an audio file first."),_=>new WaveInSource(selected.Id) };
    ...
    void BrowseAudio_Click(object sender,RoutedEventArgs e){var d=new OpenFileDialog{Title="Select an audio file",Filter="Audio files|*.mp3;*.wav;*.aac;*.m4a;*.wma;*.aiff;*.aif|All files|*.*"};if(d.ShowDialog(this)==true){audioFile=d.FileName;AudioFileText.Text=System.IO.Path.GetFileName(audioFile);}}
