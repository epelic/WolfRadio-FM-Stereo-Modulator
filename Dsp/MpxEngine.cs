...
public record MpxConfig(ushort Pi, string Ps, string RadioText, bool Rds, bool Stereo, bool CarrierOnly, bool SoftCompressor, bool TrackTitleRadioText, double PreemphasisUs, double Level);
public record MpxTelemetry(double LeftPeak, double RightPeak, double[] Spectrum);
public sealed class LiveMpxControls(double inputGain)
{
    double inputGain=inputGain;
    public double InputGain { get => Volatile.Read(ref inputGain); set => Volatile.Write(ref inputGain,value); }
}
public sealed class MpxEngine(MpxConfig cfg, IAudioSource source, IAudioSink sink, Action<MpxTelemetry>? telemetry=null, LiveMpxControls? controls=null)
{
