...
public record MpxConfig(ushort Pi, string Ps, string RadioText, bool Rds, double PreemphasisUs, double Level);
...
sealed class Preemphasis { readonly double a; double xl,xr,yl,yr; public Preemphasis(int fs,double tau)=>a=Math.Exp(-1.0/(fs*tau)); public double ProcessL(double x){yl=a*yl+x-xl;xl=x;return yl;} public double ProcessR(double x){yr=a*yr+x-xr;xr=x;return yr;} }
